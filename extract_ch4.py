...
# Chapter 4 is from index 74 to 94
doc_ch = fitz.Document()
doc_ch.insert_pdf(doc, from_page=74, to_page=94)
doc_ch.save("stat_ch4.pdf")
doc_ch.close()
...
md_text = pymupdf4llm.to_markdown("stat_ch4.pdf", write_images=True, image_path="images", image_format="png", dpi=300)
...
new_lines = []
pattern_img = re.compile(r'!\[.*?\]\((images/stat_ch4\.pdf-.*?\.png)\)')
...
pathlib.Path("stat_ch4.md").write_text(md_text, encoding="utf-8")
print(f"Extracted Chapter 4. Length: {len(md_text.splitlines())} lines.")
...
for idx, chunk in enumerate(chunks, 1):
    with open(f"stat_chunk4_{idx}.md", "w", encoding="utf-8") as f:
        f.writelines(chunk)
    print(f"stat_chunk4_{idx}.md created with {len(chunk)} lines")
