...
# Chapter 8 is from index 136 to 157
doc_ch = fitz.Document()
doc_ch.insert_pdf(doc, from_page=136, to_page=157)
doc_ch.save("stat_ch8.pdf")
doc_ch.close()
...
md_text = pymupdf4llm.to_markdown("stat_ch8.pdf", write_images=True, image_path="images", image_format="png", dpi=300)
...
new_lines = []
pattern_img = re.compile(r'!\[.*?\]\((images/stat_ch8\.pdf-.*?\.png)\)')
...
pathlib.Path("stat_ch8.md").write_text(md_text, encoding="utf-8")
print(f"Extracted Chapter 8. Length: {len(md_text.splitlines())} lines.")
...
for idx, chunk in enumerate(chunks, 1):
    with open(f"stat_chunk8_{idx}.md", "w", encoding="utf-8") as f:
        f.writelines(chunk)
    print(f"stat_chunk8_{idx}.md created with {len(chunk)} lines")
