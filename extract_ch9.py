...
# Chapter 9 is from index 158 to 174
doc_ch = fitz.Document()
doc_ch.insert_pdf(doc, from_page=158, to_page=174)
doc_ch.save("stat_ch9.pdf")
doc_ch.close()
...
md_text = pymupdf4llm.to_markdown("stat_ch9.pdf", write_images=True, image_path="images", image_format="png", dpi=300)
...
new_lines = []
pattern_img = re.compile(r'!\[.*?\]\((images/stat_ch9\.pdf-.*?\.png)\)')
...
pathlib.Path("stat_ch9.md").write_text(md_text, encoding="utf-8")
print(f"Extracted Chapter 9. Length: {len(md_text.splitlines())} lines.")

print("Splitting into 3 chunks...")
...
for idx, chunk in enumerate(chunks, 1):
    with open(f"stat_chunk9_{idx}.md", "w", encoding="utf-8") as f:
        f.writelines(chunk)
    print(f"stat_chunk9_{idx}.md created with {len(chunk)} lines")
