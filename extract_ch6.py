...
# Chapter 6 is from index 114 to 126
doc_ch = fitz.Document()
doc_ch.insert_pdf(doc, from_page=114, to_page=126)
doc_ch.save("stat_ch6.pdf")
doc_ch.close()
...
md_text = pymupdf4llm.to_markdown("stat_ch6.pdf", write_images=True, image_path="images", image_format="png", dpi=300)
...
new_lines = []
pattern_img = re.compile(r'!\[.*?\]\((images/stat_ch6\.pdf-.*?\.png)\)')
...
pathlib.Path("stat_ch6.md").write_text(md_text, encoding="utf-8")
print(f"Extracted Chapter 6. Length: {len(md_text.splitlines())} lines.")
...
for idx, chunk in enumerate(chunks, 1):
    with open(f"stat_chunk6_{idx}.md", "w", encoding="utf-8") as f:
        f.writelines(chunk)
    print(f"stat_chunk6_{idx}.md created with {len(chunk)} lines")
