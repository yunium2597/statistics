...
# Intro is from page index 0 (page 1) to 19 (page 20)
doc_intro = fitz.Document()
doc_intro.insert_pdf(doc, from_page=0, to_page=19)
doc_intro.save("stat_intro.pdf")
doc_intro.close()
...
md_text = pymupdf4llm.to_markdown("stat_intro.pdf", write_images=True, image_path="images", image_format="png", dpi=300)
...
pathlib.Path("stat_intro.md").write_text(md_text, encoding="utf-8")
print(f"Extracted Intro to stat_intro.md. Length: {len(md_text.splitlines())} lines.")
...
for idx, chunk in enumerate(chunks, 1):
    with open(f"stat_intro_{idx}.md", "w", encoding="utf-8") as f:
        f.writelines(chunk)
    print(f"stat_intro_{idx}.md created with {len(chunk)} lines")
