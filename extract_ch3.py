...
# Chapter 3 is from index 48 to 73
doc_ch = fitz.Document()
doc_ch.insert_pdf(doc, from_page=48, to_page=73)
doc_ch.save("stat_ch3.pdf")
doc_ch.close()
...
md_text = pymupdf4llm.to_markdown("stat_ch3.pdf", write_images=True, image_path="images", image_format="png", dpi=300)
...
pathlib.Path("stat_ch3.md").write_text(md_text, encoding="utf-8")
print(f"Extracted Chapter 3. Length: {len(md_text.splitlines())} lines.")

print("Splitting into 6 chunks...")
...
for idx, chunk in enumerate(chunks, 1):
    with open(f"stat_chunk3_{idx}.md", "w", encoding="utf-8") as f:
        f.writelines(chunk)
    print(f"stat_chunk3_{idx}.md created with {len(chunk)} lines")
